...
import { db, getFormattedDate, getFormattedMonth } from '../services/db';
import { useUser } from '../context/UserContext';
import * as ImagePicker from 'expo-image-picker';
import ScreenBackground from '../components/ScreenBackground';
import { parseMoneyInput } from '../utils/money';
import { Category, Wallet, Goal } from '../types';
...
export default function AddTransactionScreen({ navigation }: AddTransactionScreenProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (!user) return;
      if (type === 'savings') {
        const g = await db.getAllAsync<Goal>('SELECT * FROM goals WHERE user_id = ?', [user.id]); 
        setGoals(g); 
        if(g.length) setSelCat(g[0].id);
      } else {
        const c = await db.getAllAsync<Category>('SELECT * FROM categories WHERE type = ?', [type]); 
        setCats(c); 
        if(c.length) setSelCat(c[0].id);
      }
  ...
  const handleSave = async () => {
    ...
    if (type === 'savings') {
      ...
      await db.runAsync('UPDATE goals SET saved_amount = saved_amount + ? WHERE id = ?', [amountVal, selCat as number]);
  ...
  const doSave = async (amountVal: number) => {
    if (!user) return;
...
const styles = StyleSheet.create({
  ...
  typeRow: {flexDirection: 'row', marginBottom: 25},
  typeBtn: { flex: 1, padding: 15, borderRadius: 22, alignItems: 'center', backgroundColor: '#F8FAFC', borderWidth: 2, borderColor: '#F1F5F9', marginHorizontal: 3 }, 
  typeInc: {borderColor:'#10B981', backgroundColor:'#ECFDF5'}, 
  typeExp: {borderColor:'#F43F5E', backgroundColor:'#FFF1F2'}, 
  typeSav: {borderColor:'#4F46E5', backgroundColor:'#EEF2FF'}, 
  typeText: {fontWeight:'900'},
  chip: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F1F5F9', paddingHorizontal: 22, paddingVertical: 14, borderRadius: 20, marginRight: 12 }, 
  ...
  btn: { backgroundColor: '#4F46E5', padding: 24, borderRadius: 25, alignItems: 'center', marginTop: 10, shadowColor: '#4F46E5', shadowOpacity: 0.35, shadowOffset: { width: 0, height: 6 }, shadowRadius: 12, elevation: 5 }
});
