    ...
    <ScreenBackground>
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity style={{marginRight: 15, padding: 12, backgroundColor: '#EEF2FF', borderRadius: 20, shadowColor: '#4F46E5', shadowOpacity: 0.2, shadowOffset: {height: 3}, elevation: 5}} onPress={() => navigation.navigate('AIChat')}>
             <Ionicons name="sparkles" size={24} color="#4F46E5" />
          </TouchableOpacity>
          {user.avatar_uri ? <Image source={{ uri: user.avatar_uri }} style={styles.avatar} /> : <View style={styles.avatarPlaceholder}><Text style={{color: '#FFF', fontWeight:'bold'}}>{user.full_name.charAt(0)}</Text></View>}
        </View>
        ...
        <Animated.View style={[styles.mainCard, { opacity: anims[0], transform: [{ translateY: anims[0].interpolate({ inputRange: [0, 1], outputRange: [50, 0] }) }] }]}>
          ...
          <View style={styles.cardRow}>
             <View style={[styles.statItem, { paddingRight: 10 }]}><Ionicons name="arrow-down-circle" size={20} color="#10B981" /><Text style={styles.statText} numberOfLines={1}>Thu: {totalIncome.toLocaleString()}đ</Text></View>
             <View style={styles.statItem}><Ionicons name="arrow-up-circle" size={20} color="#F43F5E" /><Text style={styles.statText} numberOfLines={1}>Chi: {totalExpense.toLocaleString()}đ</Text></View>
          </View>
        </Animated.View>
        ...
        <Animated.View style={{ opacity: anims[2], transform: [{ translateY: anims[2].interpolate({ inputRange: [0, 1], outputRange: [50, 0] }) }] }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{paddingLeft: 25, marginBottom: 8}}>
            {goals.map(g => (
               <TouchableOpacity activeOpacity={0.9} key={g.id} style={styles.goalCard}><Text style={styles.goalName} numberOfLines={1}>{g.name}</Text><Text style={styles.goalAmount}>{g.saved_amount.toLocaleString()} / {g.target_amount.toLocaleString()}</Text><View style={styles.progressBg}><View style={[styles.progressFill, { width: `${Math.min((g.saved_amount/g.target_amount)*100, 100)}%`, backgroundColor: g.color }]} /></View></TouchableOpacity>
            ))}
          </ScrollView>
        </Animated.View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  mainCard: { backgroundColor: '#0F172A', margin: 25, marginTop: 5, padding: 30, borderRadius: 40, shadowColor: '#0F172A', shadowOpacity: 0.3, shadowRadius: 20, shadowOffset: { height: 10 }, elevation: 15 }, cardLabel: { color: '#94A3B8', fontSize: 13, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 1 }, cardBalance: { color: '#FFF', fontSize: 44, fontWeight: '900', marginVertical: 10 },
  cardRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 15, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.1)', paddingTop: 20 }, statItem: { flexDirection: 'row', alignItems: 'center', flex: 1 }, statText: { color: '#F8FAFC', fontSize: 15, fontWeight: '800', marginLeft: 6 },
  ...
  walletCard: { width: 190, minHeight: 140, padding: 25, borderRadius: 32, marginRight: 15, shadowColor: '#000', shadowOpacity: 0.15, shadowOffset: {height: 8}, shadowRadius: 15, elevation: 6 }, walletName: { color: '#FFF', fontSize: 15, opacity: 0.9, fontWeight: '600' }, walletBalance: { color: '#FFF', fontSize: 20, fontWeight: '900', marginTop: 8 },
  goalCard: { width: 180, backgroundColor: '#FFF', padding: 22, borderRadius: 32, marginRight: 15, elevation: 5 }, goalName: { fontSize: 16, fontWeight: '800', color: '#1E293B' }, goalAmount: { fontSize: 12, fontWeight: '700', color: '#64748B', marginTop: 4 }, progressBg: { height: 8, backgroundColor: '#F1F5F9', borderRadius: 4, marginTop: 15 }, progressFill: { height: 8, borderRadius: 4 }
});
