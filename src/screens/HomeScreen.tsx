...
import { getFormattedMonth } from '../services/db';
import { useUser } from '../context/UserContext';
import ScreenBackground from '../components/ScreenBackground';
import { formatCurrency } from '../utils/money';
import { Transaction, Wallet, Goal } from '../types';
import { StackNavigationProp } from '@react-navigation/stack';
import { getHomeDashboard } from '../services/finance';
...
export default function HomeScreen({ navigation }: HomeScreenProps) {
  ...
  const [currentMonth] = useState(getFormattedMonth());
  ...
  const loadData = async () => {
    if (!user) return;
    const dashboard = await getHomeDashboard(user.id, currentMonth);
    
    setWallets(dashboard.wallets);
    setGoals(dashboard.goals);
    setTxs(dashboard.transactions);
    setExchangeRates(dashboard.exchangeRates);
    setTotalIncome(dashboard.totalIncome);
    setTotalExpense(dashboard.totalExpense);
    setNetWorth(dashboard.netWorth);
    ...
    <ScreenBackground>
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity style={{marginRight: 15, padding: 12, backgroundColor: '#EEF2FF', borderRadius: 20, shadowColor: '#4F46E5', shadowOpacity: 0.3, shadowOffset: {width: 0, height: 4}, shadowRadius: 8, elevation: 5}} onPress={() => navigation.navigate('AIChat')}>
          ...
          <View>
            ...
            {(user.avatar_uri && user.avatar_uri !== 'null' && user.avatar_uri !== 'undefined' && user.avatar_uri !== '') ? <Image source={{ uri: user.avatar_uri }} style={[styles.avatar, {position: 'absolute' as const, top: 0, left: 0}]} /> : null}
          </View>
        </View>
        ...
        <Animated.View style={[styles.mainCard, { opacity: anims[0], transform: [{ translateY: anims[0].interpolate({ inputRange: [0, 1], outputRange: [50, 0] }) }] }]}>
          ...
          <Text style={styles.cardBalance} numberOfLines={1} adjustsFontSizeToFit>{formatCurrency(netWorth)}</Text>
                  ...
                  <TouchableOpacity activeOpacity={0.8} key={w.id} style={styles.walletCardWrap}>
                    <LinearGradient colors={gradientColors} start={{x:0, y:0}} end={{x:1, y:1}} style={styles.walletCard}>
                      <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start'}}>
                        <Ionicons name={w.icon as any} size={30} color="#FFF" style={{marginBottom: 10}} />
...
const styles = StyleSheet.create({
  ...
  welcomeText: { fontSize: 28, fontWeight: '900', color: '#0F172A', letterSpacing: -1 }, 
  ...
  mainCard: { backgroundColor: '#0F172A', margin: 25, marginTop: 5, padding: 30, borderRadius: 40, shadowColor: '#0F172A', shadowOpacity: 0.35, shadowRadius: 20, shadowOffset: { width: 0, height: 12 }, elevation: 15, overflow: 'hidden' }, 
  cardLabel: { color: '#94A3B8', fontSize: 13, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 1 }, 
  cardBalance: { color: '#FFF', fontSize: 42, fontWeight: '900', marginVertical: 10 },
  cardRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 15, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.1)', paddingTop: 20 }, 
  statItem: { flexDirection: 'row', alignItems: 'center', flex: 1 }, 
  statText: { color: '#F8FAFC', fontSize: 15, fontWeight: '800' },
  rowBetween: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginHorizontal: 25, marginBottom: 15 }, 
  sectionTitle: { fontSize: 22, fontWeight: '800', color: '#0F172A', letterSpacing: -0.5 }, 
  linkText: { color: '#4F46E5', fontWeight: '800', fontSize: 16 },
  walletCardWrap: { width: 190, minHeight: 140, marginRight: 15, borderRadius: 32, shadowColor: '#000', shadowOpacity: 0.15, shadowOffset: { width: 0, height: 8 }, shadowRadius: 15, elevation: 8 }, 
  walletCard: { flex: 1, padding: 25, borderRadius: 32 }, 
  walletName: { color: '#FFF', fontSize: 15, opacity: 0.9, fontWeight: '600' }, 
  walletBalance: { color: '#FFF', fontSize: 22, fontWeight: '900', marginTop: 8 },
  goalCard: { width: 180, backgroundColor: '#FFF', padding: 22, borderRadius: 32, marginRight: 15, shadowColor: '#000', shadowOpacity: 0.08, shadowOffset: { width: 0, height: 6 }, shadowRadius: 12, elevation: 5 }, 
  goalName: { fontSize: 16, fontWeight: '800', color: '#1E293B' }, 
  goalAmount: { fontSize: 12, fontWeight: '700', color: '#64748B', marginTop: 4 }, 
  progressBg: { height: 12, backgroundColor: '#E2E8F0', borderRadius: 10, marginTop: 15 }, 
  progressFill: { height: 12, borderRadius: 10 }
});
